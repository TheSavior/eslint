...
**Note:** For configuration files in your home directory, or in any path that isn't relative to the location of ESLint (either globally or locally), `extends` is resolved from the path of the project using ESLint (typically the current working directory) rather than relative to the file itself.
