 ...
 * If the config
 * @param {string} filePath The file path to calculate lookup for.
 * @param {string} configFilePath The config file referencing the file.
 * @returns {string} The lookup path for the file path.
 * @private
 */
function getLookupPath(filePath, configFilePath) {

    // calculates the path of the project including ESLint
    ...
    // require() uses __dirname as the default, so use that as fallback
    return __dirname;
}
    ...
    if (isFilePath(filePath)) {
        ...
        if (filePath.indexOf("plugin:") === 0) {
            ...
            filePath = resolveModule.sync(normalizePackageName(packagePath, "eslint-plugin"), {
                basedir: getLookupPath(filePath, relativeTo)
            });
    ...
    if (config) {
        ...
        if (config.parser) {
            config.parser = resolveModule.sync(config.parser, {
                basedir: getLookupPath(config.parser, path.dirname(path.resolve(filePath)))
            });
        }
